...
progress_bar = HTML("""
    <div style='background-color: lightgray; height: 20px'>
        <div id='progress' style='background-color: blue; width: 0%; height: 100%'></div>
    </div>
""")
...
def model_build(path, model_path, n_step_in, n_step_out, column, predicted, batch_size, epochs):
    """
    ...
    model.add(Dense(y_train.shape[predicted - 1]))
    model.compile(loss='mse', optimizer='adam')
    # print(model.summary())

    progress_callback = LambdaCallback(on_epoch_end=on_batch_end)
    model.fit(x_train, y_train, batch_size=batch_size, validation_data=(x_test, y_test), epochs=epochs,
              callbacks=[progress_callback])


if __name__ == '__main__':
    csv_path = '/Users/ctb/Downloads/PyProject/project/csv/codcr_tp.csv'
    mod_path = '/Users/ctb/Downloads/PyProject/project/deeplearn/Model/codcr_tp.h5'
    step_in = 9
    step_out = 1
    columns = 2
    predict = 2
    size = 32
    epoch = 100
    model_build(csv_path, mod_path, step_in, step_out, columns, predict, size, epoch)
